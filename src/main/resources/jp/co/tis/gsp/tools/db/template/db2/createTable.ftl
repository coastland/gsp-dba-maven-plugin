...
<#foreach column in entity.columnList>
<#if column.isAutoIncrement() && !column.isPrimaryKey()>CREATE SEQUENCE <#if entity.schema??>${entity.schema}</#if>${column.generatorKeyName} increment by 1 start with 1;
<#elseif column.isAutoIncrement() && column.isPrimaryKey()><#if column.dataType!='BIGINT' && column.dataType!='DECIMAL' && column.dataType!='FLOAT' && column.dataType!='INTEGER' && column.dataType!='NUMERIC' && column.dataType!='REAL' && column.dataType!='SMALLINT'>CREATE SEQUENCE <#if entity.schema??>${entity.schema}</#if>${column.generatorKeyName} increment by 1 start with 1;
</#if></#if>
</#foreach>