...
public class ImportSchemaMojoTest extends AbstractDdlMojoTest<ImportSchemaMojo> {
	 ...
	 */
	@Test
	@TestDBPattern(testCase = "notFoundDumpJar", testDb = { TestDB.h2 })
	public void testNotFoundDumpJar() throws Exception {
		...
		for (MojoTestFixture mf : mojoTestFixtureList) {
			...
			expected.expectMessage("PUBLIC.dmp is not found?");
	 ...
	 * <h4>検証内容</h4>
	 * <ul>
	 * <li>DB2とSQLServerの場合、import-schemaを実行すると例外が発生することをテスト。</li>
	 * </ul>
	 * 
	 * <h4>検証結果</h4>
	 * <ul>
	...
	public void testNoSupportDb() throws Exception {
		...
		for (MojoTestFixture mf : mojoTestFixtureList) {
			try {
				...
				switch (mf.testDb) {
				case db2:
					if (!msg.equals("db2を用いたimport-schemaはサポートしていません。"))
						fail();
					break;
				case sqlserver:
					if (!msg.equals("Sqlserverを用いたimport-schemaはサポートしていません。"))
						fail();
					break;
				}
			} catch (Throwable t) {
